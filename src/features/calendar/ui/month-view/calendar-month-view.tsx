...
function buildExpenseSummaryMap(
  expenseEvents: IEvent[],
): Map<string, IDayExpenseSummary> {
...
const MonthDayCell = ({
  ...
  const { startSelection, updateSelection, endSelection, isDateInSelection } =
    useDragSelect();
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-[1fr_auto_1fr] items-center px-1">
        ...
        <button
          ...
          className={cn(
            ...
            isToday(date) &&
              "bg-[var(--bg-brand)] font-bold text-primary-foreground hover:bg-[var(--bg-brand)]",
          )}
          style={{
      ...
      <div
        className={cn(
          "flex flex-col flex-1 min-h-0 gap-1",
          !currentMonth && "opacity-50",
        )}
      >
        ...
        {cellEvents.length > 0 &&
          Array.from({ length: maxVisibleEvents }, (_, i) => i).map(
            (position) => {
                ...
                >
                  {event && (
                    <MonthEventBadge
                      ...
                      onEventClick={
                        onDayClick ? () => onDayClick(date) : onEventClick
                      }
                    />
                  )}
                </div>
...
const MonthViewContent = ({
  ...
  const expenseEvents = useMemo(
    () =>
      [...singleDayEvents, ...multiDayEvents].filter(
        (e) => e.sourceType === "expense",
      ),
  ...
  const eventPositions = useMemo(
    () =>
      calculateMonthEventPositions(
        regularMultiDayEvents,
        regularSingleDayEvents,
        selectedDate,
      ),
