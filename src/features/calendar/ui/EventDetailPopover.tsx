...
const toneBg = (c: string) =>
  `color-mix(in oklab, ${c} 16%, var(--bg-surface))`;
const toneFg = (c: string) =>
  `color-mix(in oklab, ${c} 70%, var(--fg-primary))`;
...
const EventDetailPopover = ({
  ...
  const ddLabel =
    dd === 0
      ? t("detail.dday")
      : dd > 0
        ? t("detail.ddayLeft", { n: dd })
        : t("detail.ddayPast", { n: -dd });

  // 기간 — 같은 날 시간 일정만(다일은 날짜로 이미 표현), 종일은 '종일'
  const durMin =
    !event.isAllDay && sameDay ? differenceInMinutes(endDate, startDate) : null;
  const durLabel =
    ...
    <div style={{ display: "flex", flexDirection: "column" }}>
              ...
              >
                <span
                  style={{
                    ...
                    height: 7,
                    borderRadius: "var(--radius-full)",
                    background: event.calendarColor ?? color,
                  }}
                />
                {event.calendarName}
              </span>
      ...
      >
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          >
            {format(startDate, shortDateFormat, { locale })}
          </div>
          {!event.isAllDay && (
            <div
              className="num"
      ...
      <div style={{ display: "flex", flexDirection: "column" }}>
        ...
        {event.labelName && (
          <DetailRow
            icon={<Tag size={15} />}
            caption={t("form.label")}
            label={event.labelName}
            color={event.labelColor ?? color}
          />
        )}
        <DetailRow
          icon={<Repeat size={15} />}
          ...
          label={
            event.rrule ? getRruleText(event.rrule, t) : t("recurrence.none")
          }
          color={color}
        />
        {event.reminders.length > 0 && (
          <DetailRow
            ...
            label={event.reminders
              .map((r) => getReminderText(r.minutesBefore, t))
              .join(", ")}
            color={color}
          />
        )}
