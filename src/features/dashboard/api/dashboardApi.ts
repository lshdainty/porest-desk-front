...
export const dashboardApi = {
  getSummary: async (): Promise<DashboardSummary> => {
    const resp: ApiResponse<DashboardSummary> = await apiClient.get(
  ...
  getLayout: async (): Promise<LayoutResponse> => {
    const resp: ApiResponse<LayoutResponse> = await apiClient.get(
      "/v1/dashboard/layout",
    );
    return resp.data;
  },
  ...
  updateLayout: async (dashboard: string): Promise<LayoutResponse> => {
    const resp: ApiResponse<LayoutResponse> = await apiClient.patch(
      "/v1/dashboard/layout",
      { dashboard },
    );
    return resp.data;
