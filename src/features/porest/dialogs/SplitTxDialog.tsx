...
export function SplitTxDialog({
  ...
  const totalChanged =
    overrideTotal != null && overrideTotal !== recordedTotalForBadge;
  ...
  const isLoading =
    categoriesQ.isLoading || (!initialSplits && splitsQ.isLoading);

  const sameTypeCategories = useMemo(
    () =>
      (categoriesQ.data ?? []).filter(
        (c) => c.expenseType === expense.expenseType,
      ),
    [categoriesQ.data, expense.expenseType],
  );

  const [rows, setRows] = useState<Row[] | null>(null);
  const [lastApplied, setLastApplied] = useState<ReconcileStrategy | null>(
    null,
  );
  ...
  const reconcileProportional = () => {
    ...
    const scaled = safeRows.map(
      (r) => ((Number(r.amount) || 0) * targetTotal) / base,
    );
    const settled = settleRemainder(scaled, targetTotal);
    setRows(
      safeRows.map((r, i) => ({ ...r, amount: String(settled[i] ?? 0) })),
    );
    setLastApplied("prop");
  };
  ...
  const strategies = [
    {
      key: "prop" as const,
      Icon: Scale,
      title: t("splitTx.strategy.prop.title"),
      desc: t("splitTx.strategy.prop.desc"),
      recommended: totalChanged,
      onClick: reconcileProportional,
    },
    {
      key: "largest" as const,
      Icon: MoveUp,
      title: t("splitTx.strategy.largest.title"),
      desc: t("splitTx.strategy.largest.desc"),
      recommended: false,
      onClick: reconcileToLargest,
    },
    ...(remainder > 0
      ? [
          {
            key: "add" as const,
            Icon: PlusCircle,
            title: t("splitTx.strategy.add.title"),
            desc: t("splitTx.strategy.add.desc"),
            recommended: false,
            onClick: reconcileAddRow,
          },
        ]
      : []),
  ];
  ...
  const ratioSegments = safeRows.map((r) => {
    ...
    return {
      color: pal.color,
      name: cat?.categoryName ?? t("splitTx.unselected"),
      pct,
    };
  });
  ...
  return (
    ...
    >
      ...
      {safeRows.length > 0 &&
        (matched ? (
          ...
          >
            ...
            >
              {t("splitTx.splitSum")} <b className="num">{money(sumAmount)}</b>{" "}
              · {t("splitTx.total")} <b className="num">{money(targetTotal)}</b>
            </div>
          </div>
        ) : (
          <div
          ...
          >
            ...
            >
              ...
              >
                {!balanced
                  ? totalChanged
                    ? t("splitTx.totalChangedWarn")
                    : t("splitTx.mismatchWarn")
              ...
              {!balanced && (
                <>
                  ...
                  <b
                    ...
                    style={{ color: "var(--status-warning-fg)" }}
                  >
                    {remainder > 0 ? t("splitTx.short") : t("splitTx.over")}{" "}
            ...
            {!balanced && (
              <>
                ...
                >
                  <Sparkles size={14} /> {t("splitTx.quickAdjust")}{" "}
                  {quickOpen ? (
                    <ChevronUp size={14} />
                  ) : (
                    <ChevronDown size={14} />
      ...
      >
        {safeRows.map((r, idx) => {
          const cat = sameTypeCategories.find(
            (c) => c.rowId === r.categoryRowId,
          );
          const pal = getPaletteByColor(cat?.color);
          const pct =
            targetTotal > 0
              ? Math.round(((Number(r.amount) || 0) / targetTotal) * 100)
              : 0;
          return (
            <div
              key={r.uid}
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <Select
                    ...
                    onValueChange={(val) =>
                      updateRow(r.uid, { categoryRowId: Number(val) })
                    }
                  >
                    <SelectTrigger
                      style={{ background: "var(--bg-surface)", width: "100%" }}
                    >
        ...
        >
          {ratioSegments.map((seg, i) => (
            ...
            >
              ...
              <span
                className="num"
                style={{ fontWeight: "700", color: "var(--fg-primary)" }}
              >
                {seg.pct}%
              </span>
            </span>
