...
import { useMoveCategoryTransactions, useSplitCategoryIntoChild } from '@/features/expense'
...
export function CategoryMoveTxDialog({
  ...
  const [mode, setMode] = useState<'existing' | 'new'>(canSplit ? 'new' : 'existing')
  ...
  const submit = () => {
    ...
    if (mode === 'new') {
      splitMut.mutate(
        { id: source.rowId, childName: nameTrim, icon: source.icon ?? 'tag', color: source.color ?? '#9E9E9E' },
        { onSuccess: done },
      )
      return
    }
    moveMut.mutate({ id: source.rowId, targetCategoryRowId: targetRowId! }, { onSuccess: done })
  ...
  return (
    ...
    >
      ...
      {canSplit && (
        <Tabs value={mode} onValueChange={(v) => v && setMode(v as typeof mode)} className="mb-[14px]">
          <TabsList variant="pill" size="sm" className="w-full">
            <TabsTrigger value="new" className="flex-1">{t('moveTx.modeNew')}</TabsTrigger>
            <TabsTrigger value="existing" className="flex-1">{t('moveTx.modeExisting')}</TabsTrigger>
          </TabsList>
        </Tabs>
      )}
