  ...
  if (val === true)
    return (
      <Check
        size={15}
        style={{
          color: accent ? "var(--fg-brand)" : "var(--status-success-fg)",
        }}
      />
    );
  if (val === false)
...
export function SubscriptionDialog({
  ...
  const proPerMonth =
    cycle === "monthly" ? proMonthly : Math.round(proYearly / 12);
          ...
          >
            ...
            >
              {f.star && (
                <TrendingUp
                  size={13}
                  style={{ color: "var(--status-danger-fg)", flexShrink: 0 }}
                />
              )}
              {t(f.labelKey)}
            </span>
