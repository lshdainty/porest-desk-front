...
export function useLivePrices(symbols: string[], enabled: boolean): LivePrices {
  ...
  const rateByCurrency = useMemo(() => {
    ...
    rateQueries.forEach((r, i) => {
      ...
      if (c && rate != null && Number.isFinite(rate) && rate > 0) m.set(c, rate)
    })
    return m
  ...
  const candleSymbols = useMemo(() => (needsCandles ? activeSymbols : []), [needsCandles, activeSymbols])
  const candlePrevCloses = usePrevCloses(candleSymbols)

  return useMemo(() => {
    const bySymbol = new Map(
      (quotes ?? []).filter(q => Number.isFinite(q.price)).map(q => [q.symbol, q]),
    )
