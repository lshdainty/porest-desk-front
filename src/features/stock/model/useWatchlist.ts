...
export function useWatchlist(): Watchlist {
  ...
  const activeGroup = groups.find(g => g.rowId === pickedGroupId) ?? groups[0] ?? null
  ...
  const watchedSymbols = useMemo(() => new Set(groups.flatMap(g => g.items.map(i => i.symbol))), [groups])
  ...
  const toggleWatch = (symbol: string, marketCode?: string) => {
    ...
    if (groups.length === 0) {
      ...
      createGroupMut.mutate(t('watch.defaultGroupName'), {
        onSuccess: g => addItemMut.mutate({ groupId: g.rowId, symbol, marketCode }),
        onError: () => toast.error(t('watch.addFail')),
      })
      return
    }
    const groupId = activeGroupId ?? groups[0]!.rowId
    addItemMut.mutate({ groupId, symbol, marketCode }, { onError: () => toast.error(t('watch.addFail')) })
  }
