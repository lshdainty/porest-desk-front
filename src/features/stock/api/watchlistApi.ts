...
export function useRenameWatchGroup() {
  const invalidate = useInvalidateWatch()
  return useMutation({
    mutationFn: ({ groupId, groupName }: { groupId: number; groupName: string }) =>
...
export function useAddWatchItem() {
  const invalidate = useInvalidateWatch()
  return useMutation({
    mutationFn: ({ groupId, symbol, marketCode }: { groupId: number; symbol: string; marketCode?: string }) =>
...
export function findWatchEntries(groups: WatchGroup[] | undefined, symbol: string) {
  if (!groups) return []
  return groups.flatMap(g => g.items.filter(i => i.symbol === symbol).map(i => ({ group: g, item: i })))
}
