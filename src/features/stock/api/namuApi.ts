...
export const namuApi = {
  ...
  getAccounts: async (): Promise<NamuAccount[]> => {
    const resp: ApiResponse<NamuAccount[]> = await apiClient.get('/v1/namu/accounts')
    return resp.data ?? []
  },
   ...
   */
  getHoldings: async (currency = 'KRW', accountNo?: string): Promise<NamuHoldings> => {
    const resp: ApiResponse<NamuHoldings> = await apiClient.get('/v1/namu/holdings', {
