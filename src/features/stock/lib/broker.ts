...
export const brokerPath = (broker: string): string => `/desk/stocks/${broker.toLowerCase()}`

/**
 * 어느 증권사를 기본으로 열지 — 기본 소스 → 첫 연결 순.
 * 연결이 하나도 없으면 `null` (부를 곳이 없다는 뜻이지 에러가 아니다).
 */
export const defaultBroker = (connected: string[], primary: string | null | undefined): string | null => {
 ...
 */
export const useBrokerLabel = (enabled = true): ((broker: string) => string) => {
