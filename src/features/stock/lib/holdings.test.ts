...
const item = (symbol: string, evalAmount: string, profitLoss = '0', name = symbol) => ({
  symbol,
  name,
  quantity: '1',
  avgPrice: '0',
...
const holdings = (currency: string, items: ReturnType<typeof item>[]): NamuHoldings => ({
...
const KRW_ONE = holdings('KRW', [item('005930', '10000000', '1000000', '삼성전자')])
...
describe('mergeNamuHoldings', () => {
  ...
  it('수익률은 매입금액 기준으로 낸다', () => {
    ...
    const v = mergeNamuHoldings(holdings('KRW', [item('A', '11000', '1000')]), undefined, null)

    expect(v.krw?.profitRatePct).toBeCloseTo(10, 6)
  })

  it('매입금액이 0이면 수익률은 0 — 0으로 나누지 않는다', () => {
    const v = mergeNamuHoldings(holdings('KRW', [item('A', '1000', '1000')]), undefined, null)
...
describe('정렬', () => {
  ...
  it('환율이 없으면 국내 묶음 → 해외 묶음 순으로 두고 각 묶음 안에서만 정렬한다', () => {
    ...
    expect(v.rows.map(r => r.symbol)).toEqual(['KR_BIG', 'KR_SMALL', 'US_BIG', 'US_SMALL'])
  })
})

describe('krwValueOf', () => {
  it('원화 행은 그대로', () => {
    expect(krwValueOf({ ...item('A', '1000'), currency: 'KRW' }, 1400)).toBe(1000)
  })

  it('달러 행은 환율을 곱한다', () => {
    expect(krwValueOf({ ...item('A', '10'), currency: 'USD' }, 1400)).toBe(14_000)
  })
