...
export function StockBadge({ name, symbol, countryCode, size = 40 }: { name: string; symbol: string; countryCode: string; size?: number }) {
...
export type RowStock = { symbol: string; name: string; countryCode: string; currency: string }
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ fontSize: 'var(--text-badge)', color: 'var(--fg-tertiary)', display: 'flex', alignItems: 'center', gap: 5, marginTop: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
          {!hideSymbol && <span style={{ fontWeight: 600 }}>{stock.symbol}</span>}
          {sub && (
            <>
 ...
 */
export function ListWrap({ mobile, children, fill }: { mobile: boolean; children: React.ReactNode; fill?: boolean }) {
  if (mobile) return <div>{children}</div>
  return (
    <Card style={fill ? { padding: 6, flex: 1, minHeight: 0, overflowY: 'auto' } : { padding: 6 }}>{children}</Card>
  )
}
 ...
 */
export function WatchStar({ watched, onToggle }: { watched: boolean; onToggle: () => void }) {
  const { t } = useTranslation('stocks')
  return (
    <button
      type="button"
