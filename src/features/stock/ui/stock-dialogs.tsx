...
export function StockSearchDialog({
  ...
  mobile: boolean;
  onPick: (item: StockMasterItem) => void;
  onClose: () => void;
}) {
  ...
  const searched =
    debounced.length > 0 && !isFetching && q.trim() === debounced;
  return (
    <ModalShell
    ...
    >
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        ...
        <div
          style={{ maxHeight: mobile ? undefined : "56vh", overflowY: "auto" }}
        >
          {q.trim().length === 0 ? (
            <div
              style={{
...
export function WatchGroupDialog({
  mobile,
  group,
  onClose,
}: {
  ...
  const busy =
    createMut.isPending || renameMut.isPending || deleteMut.isPending;
  ...
  return (
    <ModalShell
      title={group ? t("watch.groupRename") : t("watch.groupAdd")}
      onClose={onClose}
      mobile={mobile}
    >
