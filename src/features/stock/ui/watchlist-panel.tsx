    ...
    <>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 4 }}>
          {activeGroup && (
            <Button variant="ghost" size="icon" title={t('watch.groupRename')} onClick={() => onEditGroup(activeGroup)}>
              <Pencil size={14} />
            </Button>
          )}
          <Button variant="ghost" size="icon" title={t('watch.groupAdd')} onClick={() => onEditGroup(null)}>
            <Plus size={15} />
          </Button>
        </div>
      </div>
          ...
          activeGroup.items.map(i => {
            ...
            return (
              <StockRow
                mobile={mobile}
                key={i.rowId}
                stock={{ symbol: i.symbol, name: i.nameKr, countryCode: i.countryCode, currency: i.currency }}
