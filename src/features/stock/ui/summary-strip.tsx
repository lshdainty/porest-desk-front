...
export function SummaryStrip({
  ...
  if (loading) {
    return (
      <Skeleton
        style={{ height: mobile ? 150 : 92, borderRadius: "var(--radius-lg)" }}
      />
    );
  }
          ...
          >
            {rest.map((t) => (
              <Card
                key={t.id}
                variant="bordered"
                style={{ padding: "9px 12px", flex: "none" }}
              >
  ...
  const cols = hero
    ? `1.55fr repeat(${rest.length}, minmax(0, 1fr))`
    : `repeat(${rest.length}, minmax(0, 1fr))`;
  return (
