...
const features = vi.hoisted(() => ({ current: undefined as unknown, loading: false }))

vi.mock('@/features/subscription/model/useSubscription', () => ({
  useMyFeatures: () => ({ data: features.current, isLoading: features.loading }),
}))
...
vi.mock('@/shared/ui/card', () => ({ Card: ({ children }: { children?: ReactNode }) => <div>{children}</div> }))
vi.mock('@/shared/ui/tabs', () => ({
  Tabs: ({ children }: { children?: ReactNode }) => <div data-testid="ranges">{children}</div>,
