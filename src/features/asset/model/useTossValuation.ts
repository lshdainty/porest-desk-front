...
import { useTossHoldings } from '@/features/stock/model/useTossStocks'
 ...
 * - 평가액은 토스가 종목별 평가금액을 **원화로** 내려준 값(marketValue.amount)을 그대로 쓴다.
 *   (앱 toss_dto 동일 — 별도 환율 환산하지 않음.)
...
export function useTossValuationMap(linkedAssets: Asset[]): Map<string, number> {
  ...
  return useMemo(() => {
    ...
    for (const it of items) {
      ...
      map.set(it.symbol, Math.round(amt))
    }
    return map
  }, [holdingsQ.data])
}
