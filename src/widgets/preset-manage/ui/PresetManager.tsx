...
export function PresetManager({ mobile }: { mobile: boolean }) {
  ...
  const items: ExpenseTemplate[] = useMemo(
    () => templatesQ.data ?? [],
    [templatesQ.data],
  );
  ...
  const [confirmDelete, setConfirmDelete] = useState<ExpenseTemplate | null>(
    null,
  );
  ...
  const sorted = useMemo(() => {
    ...
    if (sortBy === "name")
      arr.sort((a, b) => a.templateName.localeCompare(b.templateName, "ko"));
    if (sortBy === "recent") {
      arr.sort((a, b) =>
    ...
    >
      ...
      >
        ...
        >
          {isLoading ? (
            ...
            <>
              <PMStat
                label={t("preset.statSaved")}
                value={String(items.length)}
              />
              <PMStat
                label={t("preset.statUses")}
              ...
              const swipeActions: SwipeAction[] = [
                ...
                },
                {
                  ...
                  confirm: {
                    ...
                    message: t("preset.deleteConfirmMessage", {
                      name: p.templateName,
                    }),
      ...
      {confirmDelete && (
        <ConfirmDialog
          title={t("preset.deleteTitle")}
          message={t("preset.deleteConfirmMessage", {
            name: confirmDelete.templateName,
          })}
