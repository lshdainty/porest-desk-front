...
export function ForestReport({
  ...
  const entryByKey = useMemo(() => {
    ...
    collectionQ.data?.entries.forEach((entry) =>
      map.set(entry.constellation.constellationKey, entry),
    );
    return map;
  }, [collectionQ.data]);
  ...
  const missedOf = (ds: string): Todo[] =>
    todos.filter(
      (td) =>
        td.status !== "COMPLETED" && (td.dueDate ?? "").slice(0, 10) === ds,
    );
  ...
  const todayName = todayQ.data
    ? constellationName(todayQ.data.constellation, i18n.language)
    : "";
  ...
  const body = (
    <>
          ...
          return (
            ...
            >
              <span
                ...
                style={
                  isTodayCell
                    ? { color: "var(--fg-brand)", fontWeight: 700 }
                    : undefined
                }
              >
      ...
      <div className="frp-datehead">
        ...
        {isToday && (
          <span className="frp-datehead__asof num">
            {t("report.asOf", { time: asOf })}
          </span>
        )}
      </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <Card variant="raised" style={{ padding: "15px 16px" }}>
            <div className="frp-card__row">
              ...
              {isToday ? (
                <b>
                  {t("report.resultProgress", {
                    name: todayName,
                    lit: Math.min(pts, goal),
                    goal,
                  })}
                </b>
              ) : grownEntry ? (
                <b
                  style={{
                    color: constellationColorVar(
                      grownEntry.constellation.colorKey,
                    ),
                    display: "inline-flex",
                    alignItems: "center",
                    gap: 6,
                ...
                >
                  <ConstellationSVG
                    starMap={grownEntry.constellation.starMap}
                    size={20}
                  />
                  {t("report.resultCollected", {
                    name: constellationName(
                      grownEntry.constellation,
                      i18n.language,
                    ),
                  })}
                </b>
                ...
                <b style={{ color: "var(--fg-tertiary)" }}>
                  {selSky.guardUsed
                    ? t("report.resultWitheredGuard")
                    : t("report.resultWithered")}
                </b>
              ) : (
                <b style={{ color: "var(--fg-tertiary)" }}>
                  {t("report.resultRest")}
              ...
              {isToday && (todayQ.data?.streak ?? 0) > 0 && (
                <span className="frp-stamp">
                  <span>
                    {t("report.stampDays", { count: todayQ.data?.streak ?? 0 })}
                  </span>
                  {t("report.stampLabel")}
                </span>
              )}
          ...
          <Card variant="raised" style={{ padding: "15px 16px" }}>
            ...
            <div className="frp-tiles">
              <div className="frp-tile">
                ...
                <span className="frp-tile__num num">
                  {t("report.earnedCount", { count: pts })}
                </span>
              </div>
              <div className="frp-tile">
                <span
                ...
                <span className="frp-tile__num num">
                  {t("report.doneCount", { count: doneCount })}
                </span>
              </div>
            </div>
          </Card>
              ...
              {days.map((ds) => {
                ...
                return (
                  <div key={ds} className="frp-bars__col">
                        ...
                        PRIO_ORDER.map(
                          (k) =>
                              ...
                              <span
                                key={k}
                                style={{
                                  flex: done[k] * WEIGHT[k],
                                  background: PRIO_COLOR[k],
                                }}
                              />
                            ),
                        )}
                    ...
                    <span
                      className={`frp-bars__dow ${sel === ds ? "frp-bars__dow--sel" : ""}`}
                    >
                      {dowOf(ds)}
                    </span>
                  </div>
          ...
          {(missed.length > 0 || isToday) && (
            <Card variant="raised" style={{ padding: "15px 16px" }}>
              ...
              {missed.length === 0 ? (
                ...
                <>
                  <div className="frp-missed__count num">
                    {t("report.missedCount", { count: missed.length })}
                  </div>
                  <div
                  ...
                  >
                    {missed.map((td) => (
                      <div key={td.rowId} className="frp-missed__row">
                        <span
                          className="frp-missed__corner"
                          style={{ background: PRIO_COLOR[td.priority] }}
                        >
