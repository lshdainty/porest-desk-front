  ...
  const body = (
    <button type="button" className="frp-entry" onClick={onOpen}>
      <span className="frp-entry__art">
        <ConstellationSVG
          starMap={today.constellation.starMap}
          size={30}
          linesOnly
        />
      </span>
      <span style={{ flex: 1, minWidth: 0 }}>
        ...
        <span className="frp-entry__dots">
          {days.map((d) => {
            ...
            const color = entry
              ? constellationColorVar(entry.constellation.colorKey)
              : d.colorKey
                ? constellationColorVar(d.colorKey)
                : null;
            return (
              <span
                key={d.date}
                ...
                style={
                  !isToday && grown && color
                    ? {
                        background: `color-mix(in oklab, ${color} 16%, var(--bg-surface))`,
                        color,
                      }
                    : undefined
                }
