...
export function TxDetailDialog({
  ...
  /** [잔액 고치기]가 여는 결제계좌 — 결제계좌 없는 카드는 고칠 통장이 없다(D9). */
  const paymentAssetRowId = isCreditCard
    ? (asset?.paymentAssetRowId ?? null)
    : null;
  /**
   * 결제가 끝난 회차의 카드 거래인가(D1) — 거래 날짜와 카드의 `cardClosedThrough` 만
   * 견준다. 서버에 묻지 않는다: 확인창이 조회를 기다리거나, 못 물어 폴백 문구로 넘어가던
   * 자리를 걷었다(D4).
   */
  const closedSpan = isCreditCard
    ? closedCycleSpan(
        expense.expenseDate,
        expense.installmentMonths,
        asset?.cardClosedThrough,
      )
    : null;
  const resultContext = { closed: closedSpan != null, paymentAssetRowId };

  const handleConfirmDelete = () => {
    deleteMut.mutate(expense.rowId, {
      onSuccess: (result) => {
        // 미리 낸 돈이 돌아왔으면 그 금액을, 닫힌 회차였으면 [잔액 고치기]를(D4·D9).
        notifyResult(result, resultContext);
    ...
    refundMut.mutate(
      ...
      },
      {
        onSuccess: (updated) => {
          ...
          notifyResult(updated, resultContext);
        },
      },
    );
  };
  ...
  // 시스템이 만든 거래(매도 실현손익·이체 이자·카드 이월)는 원본을 지워야 사라진다.
  ...
  const quickActions = [
    ...
    // 시스템이 만든 거래(카드 이월·실현손익·이자)는 서버가 환불을 막는다 — 눌러야
    // 거부 토스트가 뜨는 대신 아예 감춘다(QA 23차 10).
    // 거래일이 오늘보다 뒤인 예정 거래도 감춘다 — 환불일은 거래일~오늘이라(D16) 고를 날이
    // 없다. 거래일이 지나면 나타난다(판정은 `isRefundableDate` 하나).
    ...(!isIncome &&
    !isRefunded &&
    !isAutoGenerated &&
    isRefundableDate(expense.expenseDate, todayLocalKey())
      ? [
          <DetailQuickAction
            key="refund"
            icon={Undo2}
      ...
      : []),
    // 분할도 환불된 거래·시스템이 만든 거래에는 안 띄운다 — 서버가 EXP_043·자동 생성
    // 잠금으로 막으므로 눌러 봐야 토스트만 뜬다. 열은 배열 길이로 세므로 빠지면 저절로
    // 줄어든다.
    ...(isRefunded || isAutoGenerated
      ? []
      : [
          <DetailQuickAction
            key="split"
    ...
    // 자동으로 만들어진 거래(이월 · 매도 손익 · 이체 이자)는 반복·더치페이의 원본이 될 수
    // 없다 — 이월로 반복을 만들면 매달 "이전 미결제 사용액" 이라는 일반 지출이 생겼다.
    // 숨기지 않고 끈다(사용자 결정 2026-09-22). 서버도 EXP_050 으로 거절한다.
    <DetailQuickAction
      key="recurring"
      icon={Repeat}
  ...
  return (
    <>
            ...
            {isRefunded && (
              <DetailSection>
                ...
                >
                  ...
                  {/* 결제가 끝난 회차의 환불은 통계에서만 빠진다(D1·U5) — 카드사가 실제로
                      돌려준 돈은 결제계좌 잔액을 고쳐 맞춘다. 그 폼으로 바로 간다(D9). */}
                  {closedSpan != null && paymentAssetRowId != null && (
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => navigate(editAssetPath(paymentAssetRowId))}
                    >
                      {t("closedCycle.fixBalance")}
                    </Button>
                  )}
                  <Button
                    variant="outline"
        ...
        <ConfirmDialog
          ...
          message={
            <>
              ...
              {/* 결제가 끝난 회차의 카드 거래는 지워도 통장이 그대로다(D1) — 그걸 여기서
                  말한다. 열린 회차에서 미리 낸 돈이 돌아오는 경우는 예고하지 않고 삭제
                  뒤 토스트로 알린다(D4). */}
      ...
      {/* 환불 처리 — 되돌릴 수 있으므로 danger 가 아니다. 카드면 무슨 일이
          벌어지는지 한 줄 더 알려 준다(결제가 끝난 회차면 기록만, 결제계좌가 없으면
          카드 잔액만 정리). */}
      {confirmRefund && (
        <ConfirmDialog
          ...
          message={
            <>
              ...
              <Field style={{ marginTop: 14 }}>
                ...
                {/* 환불일은 거래일부터 오늘까지다(D16) — 서버도 그 밖을 400 으로 막는다.
                    미래나 거래 전 날짜로 적으면 환불이 엉뚱한 회차·달에 잡힌다. */}
                <Input
                  id="tx-refund-date"
                  type="date"
                  min={day || undefined}
      ...
      {confirmCancelRefund && (
        <ConfirmDialog
          ...
          message={
            // 옛 환불이 만든 환급 이체가 묶여 있으면 그 이체도 되돌아간다(통장에서 다시
            // 빠진다). 그 밖은 표식만 걷히므로 "환급된 금액" 을 말하면 거짓이 된다.
            // 이체를 되돌리는 거래는 통장이 움직이므로 닫힌 회차 안내도 붙이지 않는다.
            expense.refundTransferRowId != null ? (
              t("txDetail.refundCancelConfirmTransfer")
            ) : (
              <>
                {t("txDetail.refundCancelConfirm")}
                <ClosedCycleNote span={closedSpan} />
              </>
            )
          }
