...
const state = vi.hoisted(() => ({
  ...
  connections: undefined as
    { broker: string; displayName: string }[] | undefined,
...
const subToggle = () =>
  container.querySelector<HTMLButtonElement>('[data-sidebar="menu-action"]');
...
describe("PorestSidebar 증권 하위 메뉴", () => {
  ...
  it("표시명은 서버 displayName 을 쓰고, 없으면 번역 키로 떨어진다", () => {
    ...
    render("/desk/stocks/toss");
    expect(subLinks().map((a) => a.textContent)).toEqual([
      "토스증권",
      "broker.namu",
    ]);
  });
  ...
  it("보고 있는 증권사 항목만 활성으로 표시한다", () => {
    ...
    expect(subLinks().map((a) => a.getAttribute("data-active"))).toEqual([
      "false",
      "true",
    ]);
    // aria-current 는 지금 페이지 하나에만 — 조상(증권)까지 달면 현재 위치가 둘로 읽힌다.
    expect(subLinks().map((a) => a.getAttribute("aria-current"))).toEqual([
      null,
      "page",
    ]);
