...
const DEFAULT_ASSET_COLOR = CAT_PALETTE.find(
  (p) => p.baseHex === "#2c70bf",
)!.baseHex;
...
export const AssetForm = ({
  ...
  const [cardCatalogRowId, setCardCatalogRowId] = useState<number | null>(
    asset?.cardCatalog?.rowId ?? null,
  );
              ...
              <Select
                value={
                  paymentAssetRowId != null
                    ? String(paymentAssetRowId)
                    : undefined
                }
                onValueChange={(v) =>
                  setPaymentAssetRowId(v ? Number(v) : null)
                }
                  ...
                  <SelectValue
                    placeholder={
                      bankAccounts.length === 0
                        ? t("form.noBankAccount")
                        : t("form.paymentAccountSelect")
                    }
                  />
                </SelectTrigger>
                <SelectContent>
        ...
        <div>
          ...
          <ColorSwatchGroup
            ...
            value={String(
              Math.max(
                0,
                CAT_PALETTE.findIndex((p) => p.baseHex === color),
              ),
            )}
