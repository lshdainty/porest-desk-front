...
export function CardAddDialog({ open, onClose }: CardAddDialogProps) {
  ...
  const companyColor = useMemo(
    () =>
      getBrandColor(
        selected?.company?.name ?? undefined,
  ...
  const previewName =
    nickname.trim() || selected?.cardName || t("cardAdd.newCard");
  ...
  const handleSubmit = () => {
    ...
    const outstanding = -Math.abs(
      Number(outstandingStr.replace(/[^\d-]/g, "")) || 0,
    );
    ...
    <div className="flex flex-col gap-5">
          ...
          <div className="flex items-center gap-3">
            ...
            {catalogQ.data?.meta?.totalElements != null && (
              <span className="text-[11px] text-[var(--fg-tertiary)]">
                {t("cardAdd.resultCount", {
                  count: catalogQ.data.meta.totalElements,
                })}
              </span>
            )}
          </div>
      ...
      <div>
        ...
        <Input
          ...
          onChange={(e) =>
            setOutstandingStr(e.target.value.replace(/[^\d-]/g, ""))
          }
          onBlur={() => {
