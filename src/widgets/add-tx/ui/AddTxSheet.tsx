...
export function AddTxSheet({
  ...
  /**
   * 환불 행을 **편집**하는 중인가 — 금액 칸만 잠근다.
   *
   * "환불 편집" 은 설계된 적이 없다. 의도된 길은 원거래 상세 → [환불] 로 만들고,
   * 환불 상세 → [환불 취소] 로 연결을 끊는 것뿐이다. 그런데 환불 행은 DB 상
   * 평범한 수입 행(`INCOME` + `refundOfExpenseRowId`)이라 가계부 목록에 섞이고,
   * 모든 거래에 붙는 수정 버튼을 똑같이 받는다 — 그래서 금액 칸이 열려 있었다.
   *
   * 금액이 열려 있으면 상한 안에서 자유롭게 고쳐지고, 원거래의 환불액·상계가
   * 조용히 어긋난다(서버 #330 은 상한 초과만 400 으로 막는다). 사용자 결정은
   * **"환불은 만들고 취소하는 것이지 고치는 게 아니다 — 금액이 틀렸으면 지우고
   * 다시 넣는다"** 다. 그래서 삭제는 그대로 두고 금액만 닫는다.
   *
   * 잠그는 것은 **금액 하나**다. 메모·날짜·카테고리는 환불 불변식과 무관하므로
   * 열어 둔다(자동 생성분이 날짜·자산까지 잠그는 것과 다른 이유다).
   *
   * 판정은 집계와 같은 `isRefundTx` 를 쓴다 — 조건을 다시 쓰면 한쪽만 바뀐다.
   * **새 환불을 만드는 모드(`isRefundMode`)와 겹치지 않는다** — 거기는 `expense`
   * 가 없고 금액을 쳐야 하는 자리다(상한은 `refundCap` 이 건다).
   */
  const isRefundEdit = !!expense && isRefundTx(expense);
  ...
  /**
   * 환불로 넣을 수 있는 최대 금액 — **원거래 금액 − 이미 환불된 금액**이다.
   * 환불 모드가 아니면 `null`(상한 없음, 금액 칸 공통 상한만 적용).
   *
   * 원거래 금액을 그대로 상한으로 삼으면 부분 환불을 두 번 받아 합이 원거래를 넘는다
   * (13,000원 지출에 8,000원 + 8,000원). 환불은 통계에서 지출을 **상계**하므로 합이
   * 원거래를 넘어서면 그 카테고리 지출이 마이너스로 뒤집힌다 — 그래서 원거래 금액이
   * 아니라 **남은 금액**으로 잠근다(QA #152).
   *
   * 잠그는 방식은 금액 칸이 공통으로 쓰는 것과 같다(QA #130) — 초과 입력은 토스트로
   * 알리고 그 값으로 맞춘다. 화면마다 다른 안내를 그리지 않는다.
   */
  const refundCap =
    refundOf && !isEdit
      ? // `?? 0` 은 보험이다 — 환불액이 안 실려 오면 뺄셈이 `NaN` 이 되고, `NaN` 은
        // `??` 로도 안 걸려 상한이 조용히 사라진다(목록·상세도 같은 이유로 `?? 0` 을 쓴다).
        Math.max(0, refundOf.amount - (refundOf.refundedAmount ?? 0))
      : null;
  ...
  const [amount, setAmount] = useState<string>(
    editTransfer
      ...
      : expense?.amount
        ...
        : // 환불 기본값도 **남은 금액**이다 — 원거래 금액을 채우면 부분 환불을 한 번
          // 받은 거래가 열리는 순간 상한을 넘긴 값으로 시작한다.
          refundCap
          ? String(refundCap)
          : "",
  );
  ...
  /** 금액 칸에 실제로 걸리는 상한 — 환불이면 남은 금액, 아니면 공통 상한. */
  const amountCap = refundCap ?? MAX_AMOUNT;
  /**
   * 환불 상한 초과.
   *
   * 손으로 치는 값은 `sanitizeAmountInput` 이 이미 맞추므로 여기까지 오지 않는다.
   * 그 그물을 안 지나는 경로가 하나 있다 — 외화 자동 환산(`origAmount × fxRate`)은
   * setAmount 를 직접 부른다. 그래서 저장 게이트를 따로 둔다.
   */
  const overRefundCap = refundCap != null && amountNumber > refundCap;
  /** 이미 전액 환불된 거래 — 더 넣을 금액이 없다. */
  const refundFullyDone = refundCap === 0;
  const canSave = (() => {
    if (amountNumber <= 0 || amountTooLarge || overRefundCap) return false;
    if (type === "TRANSFER") {
      return (
    ...
    >
      ...
      <Field style={{ marginBottom: 18 }}>
        ...
        <Input
          ...
          placeholder="0"
          inputMode="numeric"
          disabled={isAutoGenerated || refundFullyDone || isRefundEdit}
          style={{
        ...
        {/* 환불 상한 안내 — 초과·전액환불은 저장이 닫히므로 왜 닫혔는지 알려 준다.
        이미 일부만 환불된 거래는 남은 금액을 미리 보여 준다(초과를 치기 전에 안다). */}
        {refundCap != null &&
          (overRefundCap ? (
            <p
              style={{
            ...
            >
              {t("addTx.refundOverCap", { max: money(refundCap) })}
            </p>
          ) : refundFullyDone ? (
            <p
              style={{
            ...
            >
              {t("addTx.refundFullyDone")}
            </p>
          ) : refundOf && refundOf.refundedAmount > 0 ? (
            <p
              style={{
                margin: "6px 0 0",
                fontSize: "var(--text-caption)",
            ...
            >
              {t("addTx.refundRemaining", {
                refunded: money(refundOf.refundedAmount),
                max: money(refundCap),
              })}
            </p>
          ) : null)}
        ...
        {/* 환불 행도 같다 — 잠긴 칸에 이유가 없으면 고장으로 보인다. 여기서는
            "지우고 다시 넣어라" 까지 알려 준다(그게 사용자가 쓸 길이다).
            자동 생성분이 우선이다 — 안내가 둘이면 어느 쪽이 이유인지 모른다. */}
        {isRefundEdit && !isAutoGenerated && (
          <div className="mt-1.5 text-[11.5px] text-[var(--fg-tertiary)]">
            {t("addTx.refundAmountLocked")}
          </div>
        )}
      </Field>
