 ...
 * - JS bridge: window.PorestChart.postMessage('ready'|'error'), window.__themeBridge / __rangeBridge
 */
...
/** embed 컨텍스트 전용 axios. withCredentials=false, Bearer 헤더, 401 redirect 안 함. */
function createEmbedClient(token: string) {
...
export function EmbedStockChartPage() {
  ...
  const fetcher: CandleFetcher | undefined = useMemo(() => {
    ...
    const client = createEmbedClient(token)
