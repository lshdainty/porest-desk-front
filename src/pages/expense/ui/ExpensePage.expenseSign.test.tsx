// **부호는 값을 따른다** — 글자로 박지 않는다.
//
// 지출 합계가 음수로 온 적이 있다. 환불이 수입 행이던 때는 서버 `totalExpense` 가
// 환불을 상계해서 냈고, 지난달에 산 것을 이번 달에 환불하면 그달 지출이 음수가 됐다.
// 그런데 화면이 `−{KRW(v)}` 처럼 부호를 문자로 박아 두면 `−-51,999` 가 찍히고, 바로
// 옆 합계 칸은 `+51,999` 라 둘이 짝이 안 맞았다(QA #155).
//
// 환불이 표식으로 바뀐 뒤(합계에서 아예 빠진다) 지출 칸은 음수가 나오지 않는다. 그래도
// 이 테스트는 남긴다 — 잠그는 것은 "환불이 음수를 만든다" 가 아니라 **부호를 값에서
// 뽑는다**는 규칙이고, 그 규칙은 합계 칸과 0 에 여전히 걸려 있다. 합계 식이 또 바뀌어
// 음수가 돌아오면 여기가 먼저 잡는다.
//
