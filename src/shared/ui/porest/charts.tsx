...
export function LineChart({
  ...
  const onMove = (e: React.MouseEvent<SVGSVGElement>) => {
    ...
    const i = Math.max(
      0,
      Math.min(labels.length - 1, Math.round(localX / step)),
    );
    setHover({ i });
  };
              ...
              style={
                {
                  position: "absolute",
                  top: 8,
                  [placeRight ? "left" : "right"]:
                    `calc(${placeRight ? xPct : 100 - xPct}% + 12px)`,
...
export function BarChart({
  ...
  const onMove = (e: React.MouseEvent<SVGSVGElement>) => {
    ...
    const i = Math.max(
      0,
      Math.min(data.length - 1, Math.floor(localX / groupW)),
    );
    setHover({ i });
  };
...
export function Donut({
  ...
  const config: ChartConfig = Object.fromEntries(
    data.map((d, i) => [
      d.name || `seg-${i}`,
      { label: d.name, color: segments[i]?.color },
    ]),
  );
