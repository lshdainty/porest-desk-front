  ...
  return (
    <TabBarCtx.Provider value={{ compact }}>
      ...
      >
        {items.map((child, i) => {
          const shared =
            React.isValidElement<{ shared?: boolean }>(child) &&
            child.props.shared === true;
          const phaseCls =
            !phase || shared
              ? ""
              : phase === "out"
                ? "opacity-0 translate-y-1.5 scale-[0.92] pointer-events-none transition-[opacity,transform] duration-[130ms] ease-[var(--motion-ease-out)]"
                : "animate-[mtabIn_0.24s_var(--motion-ease-out)_both]";
          return (
            <span
              key={i}
  ...
  return (
    <button
      ...
      className={cn(
        ...
        active &&
          "font-semibold text-[var(--fg-brand-strong)] [&>svg]:text-[var(--fg-brand-strong)]",
...
export function TabBarFab({
  ...
  return (
    <button
      type="button"
      className={cn(
        "flex flex-1 items-center justify-center border-0 bg-transparent cursor-pointer",
        className,
      )}
      {...props}
    >
