...
/*
 * Porest Sheet — porest-design specs/components/sheet.md SoT 기반.
 *
 ...
 *                                     > {body}
 *                                     > SheetFooter
 * - shadow inline — Tailwind v4 --tw-shadow-* 분해가 다크모드 토큰 swap 우회 fix.
 * - bg-surface-default + p-2xl + spacing 토큰 + border-default
 */
  ...
  "fixed z-[101] gap-[var(--spacing-md)] bg-surface-default p-[var(--spacing-2xl)] transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500",
  {
    variants: {
      side: {
        ...
        left: "inset-y-0 left-0 h-full w-3/4 border-r border-border-default data-[state=closed]:slide-out-to-left data-[state=open]:slide-in-from-left sm:max-w-[24rem]",
        right:
          "inset-y-0 right-0 h-full w-3/4 border-l border-border-default data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right sm:max-w-[24rem]",
      },
    },
    defaultVariants: {
      side: "right",
...
const SheetHeader = ({
  ...
  <div
    className={cn(
      "flex flex-col gap-[var(--spacing-xs)] text-center sm:text-left",
      className,
    )}
    {...props}
  />
